---
name: nlm-skill
description: Dynamic architecture guide, profile routing rules, AI interaction protocols, and complete CLI reference for Google NotebookLM (nlm) knowledge base.
---
...
---
## 2. AI Agent Interaction Protocols
### A. On-Demand Retrieval
1. `nlm login switch <profile>`
2. `nlm notebook list --json`
3. `nlm notebook query <id> "..."` or `nlm cross query "..." --all`
4. Draft response strictly using retrieved info.
### B. Knowledge Ingestion & Save
1. Format lesson/note as clean Markdown.
2. `nlm notebook create "Title"` (if notebook doesn't exist).
3. `nlm source add <notebook_id_or_title> --text "Content" --title "Title"`
### C. Convert Studio Note to Source
1. `mcp_notebooklm_note(action="list", notebook_id="<id>")` or `nlm chat list <id> --json`
2. `mcp_notebooklm_source_add(notebook_id="<id>", source_type="text", text="<content>", title="<title>")` or `nlm source add <id> --text "<content>" --title "<title>"`
3. Do NOT delete original note unless explicitly requested.
# CLI Guide
